...
- **SSH tunnel support** (#28): connections that DBeaver reaches through an SSH tunnel are now tunneled here too, rather than connecting straight to the target host — which for a tunneled connection is the database *as seen from the SSH server*, usually `localhost`. Agent, password and public-key authentication are supported, including tunnel credentials stored separately from the database credentials in the workspace. The forward binds loopback only, is shared between the direct-query and pooled paths, and is closed on shutdown. A tunnel that cannot be opened fails with that reason instead of falling through to the recorded host.
- **SSH host key verification**: tunnel hosts are checked against `known_hosts`. A recorded host must match; an unrecorded one is accepted unless `OMNISQL_SSH_STRICT_HOST_KEY=true`.
- **Database override syntax** (#24, thanks @rrsela): a connection id may carry `\<connection\>/\<database\>` to query another database on the same server without adding a second connection. Not applicable to file-backed engines such as SQLite, where the database is a file path.
- **Multi-project workspaces**: connections are now read from every project directory in the workspace, each paired with its own credentials, instead of only `General`. `OMNISQL_PROJECT` (#27, thanks @zerafachris) still pins reading to one project, and `get_connection_info` debug output lists the projects found.
- New `OMNISQL_AWS_CLI_PATH`, `OMNISQL_IAM_TOKEN_TIMEOUT`, `OMNISQL_PROJECT`, `OMNISQL_SSH_KNOWN_HOSTS` and `OMNISQL_SSH_STRICT_HOST_KEY` environment variables.
...
- **URL-mode connections connected to localhost** (#26): a connection configured by JDBC URL keeps its host/port fields at placeholder values that DBeaver never reads. Reading them meant every URL-mode connection failed with `ECONNREFUSED`, or silently reached whatever was listening on that port locally. `configurationType` now decides which side is authoritative, as DBeaver does.
- **Host, port and database are backfilled from the JDBC URL** when a connection config omits them. URL parsing now also handles SQL Server `;databaseName=` tails, both Oracle address forms, file-backed URLs, credentials in the authority, failover host lists and IPv6 literals.
- **SQLite queries could crash the server** (#25, thanks @aneeskodappana): a missing or non-executable `sqlite3` binary emitted an unhandled `error` event — on the child process and again on its stdin — which took the whole server down instead of failing the one tool call.
- **Custom drivers registered under a UUID could resolve to the wrong dialect**: `db2` is a valid hex triple, so a UUID containing it was treated as naming a dialect and its `provider` was ignored.
